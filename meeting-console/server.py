...
def full_state() -> dict:
    ...
    return {
        ...
        "todo_open": sum(1 for a in ms.all_actions() if a["mine"] and not a["done"] and not a["dropped"] and not a["hidden"] and not a["archived"]),
...
def todos_payload() -> dict:
    ...
    for a in items:
        ...
        g["items"].append(a)
    # 남의 일(담당이 다른 사람)은 기본 화면·집계에서 뺀다 (2026-09-14). 「남의 일도 보기」 토글로만 본다
    open_items = [a for a in items if a["mine"] and not a["done"] and not a["dropped"] and not a["hidden"] and not a["archived"]]
    return {"today": today, "folders": list(by.values()), "settings": ms.todo_settings(),
            "others_open": sum(1 for a in items if not a["mine"] and not a["done"] and not a["dropped"] and not a["hidden"] and not a["archived"]),
            "no_owner_col": sum(1 for a in open_items if not a["has_owner_col"]),
            "archived_open": sum(1 for a in items if a["mine"] and a["archived"] and not a["done"] and not a["dropped"] and not a["hidden"]),
