...
# 캘린더별 ICS(iCal) 주소 찾는 법. 구글만 실제로 검증했고 나머지는 각 서비스의 공개 절차다.
#  화면은 서비스별로 접힌 목록으로 그린다 (static/setup.js).
ICS_HOWTO = [
    {"name": "구글 캘린더 (검증됨)", "steps": [
        "구글 캘린더를 브라우저에서 연다",
        "왼쪽 「내 캘린더」에서 회의가 들어오는 캘린더의 ⋮ > 「설정 및 공유」",
        "아래로 내려 「캘린더 통합」 절의 「비공개 주소의 iCal 형식」을 연다",
        "그 주소를 복사해 아래 칸에 붙여 넣는다",
    ]},
    {"name": "Outlook.com · Microsoft 365", "steps": [
        "Outlook 웹에서 ⚙ 설정 > 「캘린더」 > 「공유 캘린더」",
        "「캘린더 게시」에서 캘린더를 고르고 권한을 「모든 세부 정보를 볼 수 있음」으로 두고 「게시」",
        "생긴 링크 중 ICS 링크를 복사해 아래 칸에 붙여 넣는다",
    ]},
    {"name": "Apple iCloud 캘린더", "steps": [
        "iCloud.com 캘린더에서 캘린더 이름 옆 공유 아이콘 > 「공개 캘린더」를 켠다",
        "표시된 링크를 복사한다. webcal:// 로 시작하면 그대로 붙여도 된다 (https 로 바꿔 저장한다)",
        "공개 캘린더는 링크를 아는 사람은 누구나 볼 수 있다. 회의 캘린더만 따로 만들어 켜는 편이 안전하다",
    ]},
    {"name": "네이버 캘린더", "steps": [
        "네이버 캘린더 > 왼쪽 캘린더 목록의 ⚙ 「캘린더 관리」",
        "해당 캘린더의 상세에서 「iCal 주소」를 확인해 복사한다",
    ]},
    {"name": "그 밖의 캘린더", "steps": [
        "「iCal」「ICS」「구독 주소」「캘린더 게시」 중 하나의 메뉴가 있으면 그 주소를 쓴다",
        "주소는 이 맥에만 저장되고 화면에 다시 표시되지 않는다",
    ]},
]
...
def save_ics(url: str) -> dict:
    ...
    """
    url = re.sub(r"^webcal://", "https://", url.strip(), flags=re.I)   # iCloud 등은 webcal:// 로 준다
    url = (url or "").strip()
