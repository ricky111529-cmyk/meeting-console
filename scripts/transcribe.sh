...
# 뒤쪽 빈 녹음 자르기 (회의 폴더 안에서만. --out 으로 딴 데 저장할 때는 건드리지 않는다)
#  회의가 끝난 뒤 이어진 녹음(무음 환각 + 빈 회의실 소음)을 남겨 두고 N명으로 분리하면
#  소음이 한 덩어리를 차지해 실제 두 사람이 합쳐진다 (2026-09-08 15:00 회의 실사고).
#  텍스트(환각 문구·반복)와 음량(회의 중보다 6dB 이상 낮은 꼬리)을 함께 보고, 3분 이상일 때만 자른다.
#  원본은 original-untrimmed.*.bak 으로 남는다.
if [ -z "$OUT_FILE" ] && [ -f "$TS_FILE" ]; then
  python3 "$REPO/scripts/trim-tail.py" "$AUDIO" "$TS_FILE" "$TRANSCRIPT" || echo "⚠️ 뒤쪽 자르기 단계 실패 (원문·오디오는 그대로)" >&2
fi
